...
function listAllCourses(students) {
  // this puts all courses in one array
  const allCourses = students.map((student) => student.courses).flat();
  //this filters duplicates courses
  return (uniqueCourses = allCourses.filter(
...
function removeCourseFromStudent(student, course) {
  const courseIndex = students.indexOf(course);
  const removedCourse = student.courses.splice(courseIndex, 2);
  return student;
}
